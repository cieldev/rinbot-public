...
client.once("ready", async () => {
	...
	const cmd =  new SlashCommandBuilder()
		.setName('rin')
		.setDescription('Slash Commands(Beta)')
...
async function playVoice(message,userData,con,serverId){
    if(readFlag == false) {
		...
		if (voiceConnection) {
			...
			const data = `text=${edit}&speaker=hikari&pitch=100&speed=110`;
			const options = {
...
async function dictTimeout(editMsg, flag1, flag2) {
	embed = new Discord.MessageEmbed()
		.setFooter(`Rin - TTSBot | ${client.ws.ping}ms`, client.user.avatarURL)
...
client.on('interactionCreate', async interaction => {
	...
	if(commandName == 'help') {
		const helpembed = new Discord.MessageEmbed()
			...
			.setFooter(`Rin - TTSBot | ${client.ws.ping}ms`, client.user.avatarURL)
			.addFields(
				{name: `/rin join`, value: 'ボイスチャンネルにボットを参加させるよ。', inline: true },
				{name: `/rin leave`, value: 'ボイスチャンネルにからボットを退出させるよ。', inline: true },
				{name: `/rin voc`, value: '読み上げる声を変更するよ。', inline: true },
				{name: `/rin invite`, value: 'ボットの招待リンクを送信するよ。', inline: true },
				{name: `/rin help`, value: 'このヘルプ画面を送信するよ。', inline: true },
				{name: `/rin dicedit`, value: "辞書を確認/編集するよ。", inline: true}
			)
